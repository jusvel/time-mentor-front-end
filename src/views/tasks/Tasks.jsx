...
export default function Tasks() {
	...
	const filterTasksForDate = (date) => {
		...
		const filteredTasks = tasks.filter((task) => moment(task.deadline).format('YYYY-MM-DD') === formattedDate);
			...
			if (tasksForDate.length > 0) {
				return (
					<>
						...
						>
							<strong style={{ color: '#fff' }}>{tasksForDate.length} Task(s)</strong>
						</div>
							...
							<Box sx={modalStyle}>
								{selectedDateTasks.map((task) => (
									<div key={task.id} style={{ borderBottom: '1px solid #333' }}>
										...
										<p>Deadline: {moment(task.deadline).format('MMM Do YYYY')}</p>
									</div>
								))}
							</Box>
						</Modal>
			...
			<main className='tasks-main' style={{ height: mainHeight }}>
				...
				<div className='tasks-right'>
					<div className='tasks-top-menu'>
						<p>Tasks</p>
						<Button variant='contained' sx={{ backgroundColor: 'green' }} onClick={() => handleOpen({}, true)}>
							Add new task
						</Button>
					</div>
					{tasks.length === 0 ? (
